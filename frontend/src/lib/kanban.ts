...
// Offline fallback rendered only when the board cannot be loaded from the
// backend. The canonical seed lives in the backend (backend/db.py); this is
// just an empty, integrity-valid skeleton so the UI stays usable on error.
export const fallbackBoard: BoardData = {
  columns: [
